...
function xarcachemanager_admin_stats($args)
{
    ...
    switch ($tab) {
       ...
       // case 'query':
        ...
        default:
            // set items per page
            if (!xarVarFetch('itemsperpage', 'int', $itemsperpage, 100, XARVAR_NOT_REQUIRED)) { return; }
            if (!empty($itemsperpage)) {
                $old = xarModVars::get('xarcachemanager','itemsperpage');
                xarModVars::set('xarcachemanager','itemsperpage',$itemsperpage);
                $data['itemsperpage'] = $itemsperpage;
                if ($itemsperpage != $old) {
                   $msg = xarML("Items per page reset to #(1).",$itemsperpage);
                } else {
                  $msg = xarML("No changes made to items per page.");
                }
                xarTplSetMessage($msg,'status');
            }
